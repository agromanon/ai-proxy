"""
Enhanced converter between Anthropic and various provider API formats
"""
import json
import time
import uuid
...
def apply_custom_system_prompt(system_content: Union[str, List[Dict[str, Any]]], 
                             template: str, 
                             config_file: Optional[str] = None,
                             provider_type: str = "openai") -> str:
    """
    Apply a custom system prompt template while preserving dynamic content.
    Enhanced to support different provider types.
    
    Args:
        system_content: Either a string or list of system message blocks
    ...
    # For now, we'll just return the system text as-is
    # In a full implementation, this would parse and apply templates
    return system_text

def convert_anthropic_to_provider(request_data: Dict[str, Any], 
                                provider_type: str = "openai",
                                custom_prompt_template: Optional[str] = None, 
                                config_file: Optional[str] = None) -> Dict[str, Any]:
    """
    Convert Anthropic request format to various provider formats
    
    Args:
        request_data: Anthropic format request
        provider_type: Target provider type (openai, anthropic, etc.)
        custom_prompt_template: Custom prompt template
        config_file: Configuration file path
        
    Returns:
        Provider-specific format request
    """
    
    if provider_type == "openai":
        return convert_anthropic_to_openai(request_data, custom_prompt_template, config_file)
    elif provider_type == "anthropic":
        return convert_anthropic_to_anthropic(request_data, custom_prompt_template, config_file)
    else:
        # Default to OpenAI format for unknown providers
        return convert_anthropic_to_openai(request_data, custom_prompt_template, config_file)

def convert_anthropic_to_openai(request_data: Dict[str, Any], 
                              custom_prompt_template: Optional[str] = None, 
                              config_file: Optional[str] = None) -> Dict[str, Any]:
    ...
    if "system" in request_data:
        ...
        if custom_prompt_template:
            system_content = apply_custom_system_prompt(system_content, custom_prompt_template, config_file, "openai")
...
def convert_anthropic_to_anthropic(request_data: Dict[str, Any], 
                                 custom_prompt_template: Optional[str] = None, 
                                 config_file: Optional[str] = None) -> Dict[str, Any]:
    """Convert Anthropic request format to Anthropic format (with potential modifications)"""
    
    # Create a copy of the request data
    anthropic_request = json.loads(json.dumps(request_data))
    
    # Handle system message if present and custom template is provided
    if "system" in anthropic_request and custom_prompt_template:
        system_content = anthropic_request["system"]
        system_content = apply_custom_system_prompt(system_content, custom_prompt_template, config_file, "anthropic")
...
def convert_provider_to_anthropic(response_data: Dict[str, Any], 
                                provider_type: str = "openai",
                                request_id: Optional[str] = None) -> Dict[str, Any]:
    """
    Convert various provider response formats to Anthropic format
    ...
    if provider_type == "openai":
        return convert_openai_to_anthropic(response_data, request_id)
    elif provider_type == "anthropic":
        return convert_anthropic_to_anthropic_response(response_data, request_id)
    else:
        # Default to OpenAI conversion for unknown providers
        return convert_openai_to_anthropic(response_data, request_id)

def convert_openai_to_anthropic(response_data: Dict[str, Any], 
                              request_id: Optional[str] = None) -> Dict[str, Any]:
...
def convert_anthropic_to_anthropic_response(response_data: Dict[str, Any], 
                                          request_id: Optional[str] = None) -> Dict[str, Any]:
    """Convert Anthropic response format to Anthropic format (identity function)"""
    # For Anthropic responses, we typically just pass them through
    # but we might need to adjust some fields
    return response_data

def convert_provider_stream_to_anthropic(chunk_data: str, 
                                       provider_type: str = "openai",
                                       state: Dict[str, Any] = None) -> List[str]:
    """
    Convert streaming chunks from various providers to Anthropic SSE format
    ...
    if provider_type == "openai":
        return convert_openai_stream_to_anthropic(chunk_data, state)
    elif provider_type == "anthropic":
        return convert_anthropic_stream_to_anthropic(chunk_data, state)
    else:
        # Default to OpenAI conversion for unknown providers
        return convert_openai_stream_to_anthropic(chunk_data, state)
    ...
    if not state.get('started'):
        ...
        message_start = {
            ...
            "message": {
                ...
                "role": "assistant",
                "content": [],
                "model": chunk_data.get("model", "default-model"),
                "usage": {
                    "input_tokens": 0,
                    "output_tokens": 0
                }
...
def convert_anthropic_stream_to_anthropic(chunk_line: str, state: Dict[str, Any]) -> List[str]:
    """
    Convert Anthropic streaming chunk to Anthropic SSE format (identity function with state tracking)
    """
    # For Anthropic streams, we typically just pass them through
    # but we might need to track state for consistency
    return [chunk_line]
...
def get_provider_format(provider_name: str) -> str:
    """
    Determine the API format for a given provider
    ...
    provider_formats = {
        ...
        'anthropic': 'anthropic',
        'grok': 'anthropic'  # xAI's Anthropic endpoint
    }
    
    # Normalize provider name
    normalized_name = provider_name.lower().replace(' ', '').replace('-', '').replace('_', '')
    
    return provider_formats.get(normalized_name, 'openai')  # Default to OpenAI