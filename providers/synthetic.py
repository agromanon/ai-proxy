"""
Synthetic API provider implementation
"""
...
class SyntheticProvider(BaseProvider):
    ...
    def prepare_request(self, anthropic_request: Dict[str, Any], 
        ...
        """Convert Anthropic request to OpenAI format for Synthetic"""
    ...
    def send_request(self, provider_request: Dict[str, Any], stream: bool = False) -> requests.Response:
        ...
        try:
            ...
            raise Exception(f"Error sending request to Synthetic: {str(e)}")
    ...
    def process_stream_response(self, response: requests.Response) -> Any:
        """Process streaming response from Synthetic"""
        return response
    
    def test_connection(self) -> bool:
        """Test connection to Synthetic API"""
        try:
            headers = {
                'Content-Type': 'application/json',
                **self.get_auth_headers()
            ...
            test_request = {
                "model": self.default_model or "synthetic/default-model",
            ...
            print(f"Error testing Synthetic connection: {str(e)}")
            return False