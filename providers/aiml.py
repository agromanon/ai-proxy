...
class AIMLProvider(BaseProvider):
    ...
    def prepare_request(self, anthropic_request: Dict[str, Any], 
        ...
        """Convert Anthropic request to OpenAI format for AIML"""
    ...
    def send_request(self, provider_request: Dict[str, Any], stream: bool = False) -> requests.Response:
        ...
        try:
            ...
            raise Exception(f"Error sending request to AIML: {str(e)}")
    ...
    def process_stream_response(self, response: requests.Response) -> Any:
        """Process streaming response from AIML"""
        return response
    
    def test_connection(self) -> bool:
        """Test connection to AIML API"""
        try:
            headers = {
                'Content-Type': 'application/json',
                **self.get_auth_headers()
            ...
            test_request = {
                "model": self.default_model or "deepseek/deepseek-r1",
            ...
            print(f"Error testing AIML connection: {str(e)}")
            return False