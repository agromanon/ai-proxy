...
class ChutesProvider(BaseProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Ensure correct endpoint for LLM models (not the platform API)
        if 'llm.chutes.ai' not in self.api_endpoint:
            self.api_endpoint = 'http://llm.chutes.ai/api/v1'
    
    def prepare_request(self, anthropic_request: Dict[str, Any], 
                       custom_prompt_template: Optional[str] = None,
                       prompt_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Convert Anthropic request to OpenAI format for Chutes"""
    ...
    def send_request(self, provider_request: Dict[str, Any], stream: bool = False) -> requests.Response:
        """Send request to Chutes LLM API"""
        headers = {
        ...
        try:
            ...
            raise Exception(f"Error sending request to Chutes: {str(e)}")
    ...
    def process_stream_response(self, response: requests.Response) -> Any:
        """Process streaming response from Chutes"""
        return response
    
    def test_connection(self) -> bool:
        """Test connection to Chutes LLM API"""
        try:
            headers = {
                'Content-Type': 'application/json',
                **self.get_auth_headers()
            ...
            test_request = {
                "model": self.default_model or "chutes/default-model",
            ...
            print(f"Error testing Chutes connection: {str(e)}")
            return False